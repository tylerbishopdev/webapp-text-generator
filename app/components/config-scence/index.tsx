...
const ConfigSence: FC<IConfigSenceProps> = ({
    ...
    <form>
  {
    promptConfig.prompt_variables.map(item => (
      ...
      <label className='text-sm font-light text-zinc-900' > { item.name } </label>
    < div className = 'mt-2' >
    {
      item.type === 'select' && (
        <Select
                ...
                )}
{
  item.type === 'string' && (
    <input
                    type="text"
  className = "block w-full p-2 text-gray-900 border rounded-lg border-zinc-700 bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 "
...
disabled = { false}
  >
  <PlayIcon className="w-4 h-4 mr-1 shrink-0" aria - hidden="true" />
